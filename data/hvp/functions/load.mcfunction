team add Hunters {"text":"Hunters","color":"red"}
team add Player {"text":"Player","color":"blue"}

team modify Hunters color red
team modify Player color blue

team modify Hunters nametagVisibility always
team modify Player nametagVisibility never

scoreboard objectives add be_player trigger
scoreboard objectives add player_pos dummy

scoreboard objectives add X dummy
scoreboard objectives add Y dummy
scoreboard objectives add Z dummy

scoreboard objectives add X_diff dummy
scoreboard objectives add Y_diff dummy
scoreboard objectives add Z_diff dummy

scoreboard objectives add distance dummy

scoreboard objectives add sqrtI dummy
scoreboard objectives add sqrtX dummy
scoreboard objectives add realDistance dummy

scoreboard objectives add dimension dummy

scoreboard objectives add deaths deathCount

scoreboard objectives add spawn_x dummy "Spawn X"
scoreboard objectives add spawn_y dummy "Spawn Y"
scoreboard objectives add spawn_z dummy "Spawn Z"